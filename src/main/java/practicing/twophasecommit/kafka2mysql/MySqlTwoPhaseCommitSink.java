package practicing.twophasecommit.kafka2mysql;
 ...
 * @author altenchen
 * @time 2021/2/24
 * @description 功能 自定义 Kafka to mysql,继承TwoPhaseCommitSinkFunction,实现两阶段提交
 *
 *              说明 使用关系型数据库 MySQL，开启 CheckPoint 机制的前提下，为了保证前一次 CheckPoint
 *                  成功后到这次 CheckPoint 成功之前这段时间内的数据不丢失，如果执行到一半过程任务失败
 *                  了，从而导致前一次CheckPoint成功后到任务失败前的数据已经存储到了MySQL,然而这部分数
 *                  据并没有写入到 CheckPoint。如果任务重启后，前一次CheckPoint成功后到任务失败前的数
 *                  据便会再次写入MySQL,从而导致数据重复的问题。这种情况，便使用到了
 *                  TwoPhaseCommitSinkFunction类，以此来实现 MySQL 关系型数据库的二阶提交。
 */
