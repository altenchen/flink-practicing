...
public class CustomParquetAvroWriters {
    ...
    public static <T extends SpecificRecordBase> ParquetWriterFactory<T> forSpecificRecord(
            Class<T> type, boolean isCompress, CompressionCodecName compressionCodecName) {
        final String schemaString = SpecificData.get().getSchema(type).toString();
        final ParquetBuilder<T> builder =
                (out) -> createAvroParquetWriter(schemaString, SpecificData.get(), out, isCompress, compressionCodecName);
     ...
     */
    public static ParquetWriterFactory<GenericRecord> forGenericRecord(Schema schema, boolean isCompress, CompressionCodecName compressionCodecName) {
        final String schemaString = schema.toString();
        final ParquetBuilder<GenericRecord> builder =
                (out) -> createAvroParquetWriter(schemaString, GenericData.get(), out, isCompress, compressionCodecName);
        return new ParquetWriterFactory<>(builder);
    }
     ...
     * @param type The class of the type to write.
     */
    public static <T> ParquetWriterFactory<T> forReflectRecord(Class<T> type, boolean isCompress, CompressionCodecName compressionCodecName) {
        final String schemaString = ReflectData.get().getSchema(type).toString();
        final ParquetBuilder<T> builder =
                (out) -> createAvroParquetWriter(schemaString, ReflectData.get(), out, isCompress, compressionCodecName);
        return new ParquetWriterFactory<>(builder);
    }
    ...
    private static <T> ParquetWriter<T> createAvroParquetWriter(
        ...
        if (isCompress) {
            return AvroParquetWriter.<T>builder(out)
                    ...
                    .withCompressionCodec(compressionCodecName) //自定义压缩算法
                    .build();
        } else {
            return AvroParquetWriter.<T>builder(out)
                    .withSchema(schema)
